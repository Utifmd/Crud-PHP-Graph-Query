...
if (isset($_GET['Nobp'])) {
    $kd_del = $_GET['Nobp'];
    $Qdelete = mysql_query("DELETE FROM tmhs WHERE Nobp='$kd_del'");
    if ($Qdelete) {
...
$Qupdate = mysql_query("SELECT * FROM tmhs WHERE Nobp='$kd_updt' ");
...
if (isset($_POST['btn_updt'])) {
    ...
    $Qupdt = mysql_query("UPDATE tmhs SET Nama='$nama', Jekel='$jk', Tempat='$tempat', TglLahir='$tgllahir', Kelas='$kelas', Kode_f='$kodef', alamat='$alamat' WHERE Nobp='$kd_updt' ");
    if ($Qupdt) {
...
<head>
	...
	<script type="text/javascript" src="js/jquery.fusioncharts.debug.js"></script>
	<SCRIPT LANGUAGE="javascript" src="js/jquery.fusioncharts.js"></SCRIPT>
...
<td> : </td>
<td>
<input type="text" value="<?php echo $data_mhs[''] ?>" name="txtnobp" size="16" maxlength="14">
</td>
</tr>
...
<td> : </td>
<td>
<input type="text" name="txtnama" size="30" maxlength="30">
</td>
</tr>
...
<td> : </td>
<td>
<input type="text" name="txttmptlahir" size="16" maxlength="16"> /
<?php
...
<tr>
<td><label>mhs</label></td>
<td> : </td>
<td>
<select name=mhs>
<option value=0 selected>-Pilih mhs-</option>
<?php
include "/Library/connectdb.php";
$sqlmhs =mysql_query("select * from tmhs order by Nm_mhs");
while($data=mysql_fetch_array($sqlmhs))
{
echo "<option value=$data[Nm_mhs]>$data[Nm_mhs]</option>";
}
?>
</select>
</td>
...
<td> : </td>
<td>
<textarea name="txtalamat" cols="60" rows="3"></textarea>
</td>
</tr>
...
<!-- 
<id id="myChart1Container"></id>
<?php
//include "tampil_grafik.php";
?> -->
